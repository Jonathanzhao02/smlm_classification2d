...
class GridAlignment():
    def __init__(self, grid, points, weights=None, recenter=False):
        self.grid = np.copy(grid)
        self.points = np.copy(points)

        if weights is None:
            self.weights = np.ones(points.shape[0])
        else:
    ...
    def squareNNDist(self,tm):
        ...
        return np.sum(np.multiply(self.nn[0],self.weights)**2, dtype=np.float64) / self.points.shape[0]
